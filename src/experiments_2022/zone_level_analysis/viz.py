...
def make_dot_plot(
    ...
    for this_key in these_keys:
        ...
        if sort_by == "index":
            ...
            this_df = this_df.sort_values(by="numeric_index").drop(
            ...
            this_df.sort_values(sort_by, inplace=True)
    ...
    if these_color_keys is not None:
        showlegend = True
        for this_color_key_i in range(len(these_color_keys)):
    ...
    if shape_data is not None:
        ...
        for this_shape_key_i in range(len(these_shape_keys)):
